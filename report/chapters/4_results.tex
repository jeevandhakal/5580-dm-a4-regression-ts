...
The results demonstrate a clear hierarchy in predictive stability, as visualized in \autoref{fig:mape_rmse_dist}. \textbf{LightGBM} achieved the lowest error (9.88\% MAPE), benefiting from its histogram-based gradient boosting which effectively handles high-frequency temporal spikes.

\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\linewidth]{figures/model_mape_rmse.png}
    \caption{Model Performance: Mean Absolute Percentage Error (MAPE) and Root Mean Squared Error (RMSE) comparison across all candidates.}
    \label{fig:mape_rmse_dist}
\end{figure}
